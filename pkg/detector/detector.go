...
func (d *ResourceDetector) Start(ctx context.Context) error {
	...
	policyWorkerOptions := util.Options{
		...
		KeyFunc:       PropagationPolicyKeyFunc,
...
func (d *ResourceDetector) OnPropagationPolicyUpdate(oldObj, newObj interface{}) {
	...
	if features.FeatureGate.Enabled(features.PolicyPreemption) {
		oldPolicy, ok := oldObj.(*policyv1alpha1.PropagationPolicy)
		if !ok {
			klog.Errorf("Failed to convert old object to PropagationPolicy, objectType: %T", oldObj)
			return
		}
		newPolicy, ok := newObj.(*policyv1alpha1.PropagationPolicy)
		if !ok {
			klog.Errorf("Failed to convert new object to PropagationPolicy, objectType: %T", newObj)
			return
		}
...
func (d *ResourceDetector) OnClusterPropagationPolicyUpdate(oldObj, newObj interface{}) {
	...
	// which can be used to detect priority changes during reconcile logic.
	if features.FeatureGate.Enabled(features.PolicyPreemption) {
		oldPolicy, ok := oldObj.(*policyv1alpha1.ClusterPropagationPolicy)
		if !ok {
			klog.Errorf("Failed to convert old object to ClusterPropagationPolicy, objectType: %T", oldObj)
			return
		}
		newPolicy, ok := newObj.(*policyv1alpha1.ClusterPropagationPolicy)
		if !ok {
			klog.Errorf("Failed to convert new object to ClusterPropagationPolicy, objectType: %T", newObj)
			return
		}
