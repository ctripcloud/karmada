...
func (c *Controller) syncWork(key util.QueueKey) error {
	...
	group := util.AddStepForRequestObject("execution-controller", req, &workv1alpha1.Work{})
	klog.Infof("[Group %s] Begin to sync work %s/%s", group, req.Namespace, req.Name)
...
func (c *Controller) syncToClusters(clusterName string, work *workv1alpha1.Work, group string) error {
	...
	if len(errs) > 0 {
		...
		err := c.updateAppliedConditionIfNeed(work, metav1.ConditionFalse, "AppliedFailed", message, group)
		if err != nil {
	...
	err := c.updateAppliedConditionIfNeed(work, metav1.ConditionTrue, "AppliedSuccessful", "Manifest has been successfully applied", group)
	if err != nil {
		klog.Errorf("[Group %s] Failed to update applied status for given work %v, namespace is %v, err is %v", group, work.Name, work.Namespace, err)
		return err
	}
...
func (c *Controller) updateAppliedConditionIfNeed(work *workv1alpha1.Work, status metav1.ConditionStatus, reason, message, group string) error {
