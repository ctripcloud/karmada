...
func (c *WorkStatusController) syncWorkStatus(key util.QueueKey) error {
	...
	if err != nil {
		if apierrors.IsNotFound(err) {
			...
			klog.Warningf("[Group %s] Skip reflecting %s(%s/%s) status to Work(%s/%s) for not applied successfully yet.",
				group, fedKey.Kind, fedKey.Namespace, fedKey.Name, executionSpace, workName)
			return nil
		}
		return err
	}
