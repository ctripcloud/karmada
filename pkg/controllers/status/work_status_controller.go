...
func (c *WorkStatusController) onAdd(obj any, isInitialList bool) {
	curObj := obj.(runtime.Object)
	if priorityWorker, ok := c.worker.(util.AsyncPriorityWorker); ok {
		priority := 0
		if isInitialList {
			priority = util.LowPriority
		}
		priorityWorker.EnqueueWithOpts(util.AddOpts{Priority: priority}, curObj)
		return
	}
